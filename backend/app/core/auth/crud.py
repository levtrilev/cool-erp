...
from app.core.auth.schemas import UserRegisterSchema, UserUpdateSchema

# Глобальное хранилище сессий (в памяти: {session_id: {"user_id": UUID, ...}})
sessions_storage: dict[str, dict[str, Any]] = {}


class CRUDUser(CRUDBase[UserModel, UserRegisterSchema, UserUpdateSchema]):
    ...
    # ) -> UserModel:
    #     """Обновление с авто-хешированием пароля, авто-коммитом и очисткой сессий."""
    ...
    #     db.add(
    #         db_user
    #     )  # В SQLAlchemy для обновления измененного объекта используется add()
    #     await db.commit()
    #     await db.refresh(db_user)

    #     # 5. ПРИНУДИТЕЛЬНЫЙ РАЗЛОГИН: удаляем все сессии данного пользователя
    #     # Проходим по копии словаря, чтобы избежать ошибок RuntimeError при изменении размера словаря во время итерации
    #     for session_id, session_data in list(sessions_storage.items()):
    #         if session_data.get("user_id") == db_user.id:
